...
data-bind="foreach: <observableArray>, selection: { data: <observableArray>, focused: <observable>, mode: 'single', properties: { selected: 'selected', focused: 'focused'} }"
*/
    ...
    return ko.bindingHandlers.selection = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            if (bindingValue.data) {
                ...
                mode = (bindingValue.mode && ['multi', 'simple', 'single'].indexOf(bindingValue.mode.toLowerCase()) !== -1) ? bindingValue.mode.toLowerCase() : mode;
            ...
            selection.subscribe(function (newSelection) {
                if (mode === 'single' && newSelection.length > 1) {
            ...
            function createSingleModeEventMatchers() {
                ...
                matchers.click.register(
                    // click
                    { which: 1 }, function (event, item) {
                    selectItem(item);
                });

                matchers.key.register(
                    // space
                    { which: 32 }, function (event, item) {
                    toggleSelection(item);
                });

                matchers.key.register(
                    // end
                    { which: 35 }, function (event, item) {
                    selectItem(lastItem());
                });

                matchers.key.register(
                    // home
                    { which: 36 }, function (event, item) {
                    selectItem(firstItem());
                });

                matchers.key.register(
                    // up arrow
                    { which: 38 }, function (event, item) {
                    selectItem(previousItem(item));
                });

                matchers.key.register(
                    // down arrow
                    { which: 40 }, function (event, item) {
            ...
            function createSimpleModeEventMatchers() {
                var matchers = {
                  click: new EventMatcher(),
                  key: new EventMatcher()
                };
            ...
            function createMultiModeEventMatchers() {
                ...
                matchers.click.register(
                    // <shift> click
                    { which: 1, shiftKey: true }, function (event, item) {
                ...
                matchers.key.register(
                    // <shift> end
                    { which: 35, shiftKey: true }, function (event, item) {
                ...
                matchers.key.register(
                    // <shift> home
                    { which: 36, shiftKey: true }, function (event, item) {
                ...
                matchers.key.register(
                    // <shift> up arrow
                    { which: 38, shiftKey: true }, function (event, item) {
                ...
                matchers.key.register(
                    // <shift> down arrow
                    { which: 40, shiftKey: true }, function (event, item) {
            ...
            if (mode === 'single') {
              matchers = createSingleModeEventMatchers();
            } else if (mode === 'simple') {
              matchers = createSimpleModeEventMatchers();
            } else {
              matchers = createMultiModeEventMatchers();
            }
